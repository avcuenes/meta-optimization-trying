...
class IWOA(Optimizer):
    """
    The original version of: Improved Whale Optimization Algorithm (IWOA)

    Links:
        1. https://downloads.hindawi.com/journals/ddns/2021/8832251.pdf

    Examples
    ~~~~~~~~
    >>> import numpy as np
    >>> from mealpy.swarm_based.WOA import IWOA
    >>>
    >>> def fitness_function(solution):
    >>>     return np.sum(solution**2)
    >>>
    ...
    >>> epoch = 1000
    >>> pop_size = 50
    >>> model = IWOA(problem_dict1, epoch, pop_size)
    >>> best_position, best_fitness = model.solve()
    >>> print(f"Solution: {best_position}, Fitness: {best_fitness}")
    ...
    def evolve(self, epoch):
        ...
        if epoch < self.epoch*0.7:
            a = pow(1-epoch/self.epoch)
        else:
